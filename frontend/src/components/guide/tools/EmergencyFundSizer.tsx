          ...
          <dl className="tool__facts tool__facts--wide">
            ...
            <dd className="tabular">
              {data.essentials_monthly !== null ? (
                ...
                <>
                  not known yet — <Link to="/reports?tab=essentials">tag what you could not do without as Essential</Link>
                </>
              )}
            </dd>
            <dt>Target, {data.months} {data.months === 1 ? 'month' : 'months'}</dt>
            <dd className="tabular">{data.target !== null ? formatMoney(Number(data.target)) : '—'}</dd>
            <dt>Emergency fund today</dt>
            <dd className="tabular">
              {data.current !== null ? formatMoney(Number(data.current)) : 'nothing found — correct it on the roadmap'}
            </dd>
            <dt>Still to save</dt>
            ...
            <dd>
              ...
              {data.months_to_fund === null && data.gap !== null && Number(data.gap) > 0 && 'put a monthly amount above to see a date'}
