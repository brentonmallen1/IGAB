...
describe('checkup copy', () => {
  ...
  it('status: a fired finding wins, then the figure against its target', () => {
    expect(metricStatus(metric({}), finding('ef_below_starter'))).toEqual({
      status: 'warn',
      text: 'worth a look',
    })
    ...
    expect(
      metricStatus(metric({ key: 'high_interest_debt', value: '0', target: '0', unit: 'money' }))
        .text
    ).toBe('none')
    expect(
      metricStatus(metric({ key: 'categories_funded', value: '18', target: '21', unit: 'count' }))
        .text
    ).toBe('3 underfunded')
    expect(
      metricStatus(metric({ key: 'categories_funded', value: '21', target: '21', unit: 'count' }))
        .text
    ).toBe('all funded')
  })

  it('only an emergency fund that has not started speaks in red', () => {
    expect(metricStatus(metric({ value: '0' }), finding('ef_not_started'))).toEqual({
      status: 'danger',
      text: 'not started',
    })
  ...
  it('says what the months are worth, when the row carries the money', () => {
    ...
    expect(formatMoneyLine(row, fmt, { emergency_fund_months: 3 })).toBe(
      '$1240 of $9720 — three months of essentials'
    )
  ...
  it('progress only where a bar means something', () => {
    ...
    expect(
      metricProgress(metric({ key: 'high_interest_debt', value: '3410', target: '0' }))
    ).toBeNull()
  ...
  it('formats values and targets by unit', () => {
    ...
    expect(
      formatMetricTarget(metric({}), fmt, {
        emergency_fund_months: 3,
        emergency_fund_months_high: 6,
      })
    ).toBe('target 3–6 months')
    expect(
      formatMetricTarget(metric({ key: 'high_interest_debt', unit: 'money', target: '0' }), fmt, {})
    ).toBe('target: none')
    expect(
      formatMetricTarget(metric({ key: 'categories_funded', unit: 'count', target: '21' }), fmt, {})
    ).toBe('of 21 with targets')
    expect(formatMetricTarget(metric({ unit: 'money', target: '1000' }), fmt, {})).toBe(
      'target $1000 to start'
    )
  })
})
