...
describe('checkup copy', () => {
  ...
  it('status: a fired finding wins, then the figure against its target', () => {
    ...
    expect(
      metricStatus(metric({ key: 'high_interest_debt', value: 0, target: 0, unit: 'money' })).text
    ).toBe('none')
    expect(
      metricStatus(metric({ key: 'categories_funded', value: 18, target: 21, unit: 'count' })).text
    ).toBe('3 underfunded')
    expect(
      metricStatus(metric({ key: 'categories_funded', value: 21, target: 21, unit: 'count' })).text
    ).toBe('all funded')
  })
