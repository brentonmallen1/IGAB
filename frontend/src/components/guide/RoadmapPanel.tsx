...
import { ROADMAP_STEPS, ROADMAP_ATTRIBUTION, ROADMAP_DISCLAIMER } from '../../content/roadmap'
    ...
    <div className={`guide-roadmap ${isMap ? 'guide-roadmap--map' : ''}`}>
        ...
        <div className="guide-roadmap__heading">
          ...
          {!isMap && (
            <p className="guide-roadmap__lede">
              A common order for financial priorities — cover the essentials, build a buffer, clear
              expensive debt, then save for what comes next.
            </p>
          )}
        </div>
      ...
      {dialog === 'source' && (
        ...
        >
          ...
          <p className="dialog__body">
            The decisions are theirs. The wording here is ours: rewritten to be shorter, to say what
            each step means inside IGAB, and to work on a phone. Figures that change from year to
            year are deliberately left out, so nothing here quietly goes stale.
          </p>
          <a
