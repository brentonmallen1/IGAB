...
describe('the paid-ahead chip', () => {
  // Ready to Assign is reduced by money paid toward cards past their
  // envelopes (a card's Set aside enters the total signed, so without the
  // correction the payment never showed). A number that moved needs its
  // reason beside it; the chip is the reason, and it opens the cards.
  it('appears with the served figure, and only when there is one', () => {
    month.current = {
      to_be_assigned: 850,
      paid_ahead_on_cards: 50,
      cards: [],
    } as unknown as BudgetMonth
    render(<TbaHero budgetId="b1" month="2026-08-01" />)
    expect(screen.getByText('paid ahead on cards').parentElement?.textContent).toBe(
      '-$50.00paid ahead on cards'
    )
  })

  it('opens the explanation naming each card', () => {
    month.current = {
      to_be_assigned: 850,
      paid_ahead_on_cards: 50,
      cards: [
        { account_id: 'a', name: 'Sapphire Visa', paid_ahead_unmirrored: 50 },
        { account_id: 'b', name: 'Harborstone Card', paid_ahead_unmirrored: 0 },
      ],
    } as unknown as BudgetMonth
    render(<TbaHero budgetId="b1" month="2026-08-01" />)
    fireEvent.click(screen.getByText('paid ahead on cards').closest('button')!)
    const dialog = screen.getByRole('dialog', { name: 'Paid ahead on cards' })
    expect(dialog).toBeInTheDocument()
    expect(dialog.textContent).toMatch(/already reflects it/)
    // Only cards with a share are listed.
    expect(dialog.textContent).toContain('Sapphire Visa')
    expect(dialog.textContent).not.toContain('Harborstone Card')
  })
})
