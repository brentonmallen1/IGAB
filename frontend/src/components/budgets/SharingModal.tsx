...
export function SharingModal({
  ...
  async function handleAdd(e: React.FormEvent) {
    e.preventDefault()
    if (!selectedUserId) {
      setAddError('Pick someone to share with')
      return
    }
    setAddError(null)
    try {
  ...
  return (
    <Dialog title={`Sharing — ${budgetName}`} onClose={onClose} historyKey="budget-sharing">
      <div className="sharing-modal__body">
        {isLoading ? (
          <div className="sharing-modal__hint">Loading…</div>
        ) : (
              ...
              return (
                <div key={m.user_id} className="sharing-modal__member">
                  ...
                  {canRemove && (
                    <button
                      type="button"
                      className="dialog-btn dialog-btn--danger sharing-modal__icon-btn"
        ...
        {isOwner &&
          (addable.length === 0 ? (
            <div className="sharing-modal__add sharing-modal__hint">
              Everyone in the household already has access. New people are added in System → Users
              first.
            </div>
          ) : (
            <form className="dialog-form sharing-modal__add" onSubmit={handleAdd}>
              <div className="sharing-modal__add-row">
                <select
                ...
                <button
                  type="submit"
                  className="dialog-btn dialog-btn--primary sharing-modal__icon-btn"
              ...
              {addError && <p className="dialog-form__error">{addError}</p>}
            </form>
          ))}
      </div>
    </Dialog>
