  ...
  // ── Step 3 (first pass) ───────────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        body: 'Categorize everything the card syncs, then assign what you can to the card itself each month. Old debt sits calmly as not covered until you cover it.',
        detail:
          "Categorizing a card transaction never takes money you do not have. Money moves from the category into the card's Set aside only up to what the category could actually cover; any shortfall rides on the card as debt not covered, and Ready to Assign never hears about it. So categorize everything — the spending reports come free, and the debt cannot charge you twice.\n\nThe monthly loop: assign what you can afford to the card in the Credit cards section (Ready to Assign goes down, the debt not covered goes down, one for one), then pay the card with a transfer from checking — the transfer drains Set aside and the balance together. Pay no more than is set aside: past it the card is overspent, and whatever you have not assigned by month end comes out of next month's Ready to Assign. A paydown target on the card row keeps the number honest month to month.\n\nEnter the card's APR and minimum payment on its liability page. That is what places the card in the payoff planner's avalanche and snowball schedules, and what lets the checkup call it high-interest debt instead of guessing.",
