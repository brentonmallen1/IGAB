...
async def _unpaired_legs(
    ...
    "Sapphire Card" envelope, the card leg to whatever auto-categorization guessed
...
async def _report_unfinished_migration(session: AsyncSession, budget_id: uuid.UUID) -> None:
    ...
    budgeted card payments the pre-card-account way — a hand-made "Sapphire Card"
    ...
    of -4,075.00 beside an envelope holding +4,182.50 is not a state to hand
