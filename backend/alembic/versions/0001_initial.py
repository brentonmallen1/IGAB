...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "last_reconciled_balance", sa.Numeric(precision=19, scale=4), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["category_group_id"], ["category_groups.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["transfer_account_id"], ["accounts.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    # Transactions (includes sync_id and sync_source from the start)
    op.create_table(
        "transactions",
        sa.Column("id", sa.UUID(), nullable=False),
        sa.Column("budget_id", sa.UUID(), nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ["parent_transaction_id"], ["transactions.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["payee_id"], ["payees.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(
            ["transfer_id"], ["transactions.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    # Dedup backstop: at most one LIVE row per bank/import identity per account.
    op.create_index(
        "uq_transactions_account_sync_id",
        "transactions",
        ["account_id", "sync_id"],
        unique=True,
        postgresql_where=sa.text("sync_id IS NOT NULL AND NOT is_deleted"),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "statement_balance", sa.Numeric(precision=19, scale=4), nullable=False
        ),
        sa.Column("cleared_balance", sa.Numeric(precision=19, scale=4), nullable=False),
        sa.Column(
            "adjustment_amount", sa.Numeric(precision=19, scale=4), nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(["account_id"], ["accounts.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["adjustment_transaction_id"], ["transactions.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["manual_transaction_id"], ["transactions.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["synced_transaction_id"], ["transactions.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
...
def downgrade() -> None:
    ...
    op.drop_index("uq_transactions_account_import_id", "transactions")
    op.drop_index("uq_transactions_account_sync_id", "transactions")
