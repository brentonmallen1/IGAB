...
async def test_a_card_anchor_lands_as_uncovered_debt(db_session):
    ...
    svc = _service(services, payload, balances={"sf-visa": Decimal("-6925.97")})
    ...
    assert result.get("error") is None, result
    assert result["anchored"] == 1
    assert await services.account_repo.get_balance(card.id) == Decimal("-6925.97")
    ...
    assert row.balance == Decimal("-6925.97")
    assert row.uncovered == Decimal("6925.97")
