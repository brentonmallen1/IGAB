...
async def test_unmanaged_liability_reduces_net_worth_exactly_once(db_session):
    ...
    liability = await create_liability(
        db_session, budget, "Family", manual_balance=Decimal("1200.00")
    )
    await create_liability_snapshot(
        db_session, liability, TODAY, Decimal("1200.00"), source="initial"
    )
    ...
    assert history[-1]["unmanaged_liability_total"] == Decimal("1200.00")
    assert history[-1]["total_liabilities"] == Decimal("1200.00")


async def test_managed_liability_is_not_double_counted(db_session):
    report_svc = ReportService(db_session)
    user = await create_user(db_session)
    budget = await create_budget(db_session, user)
    checking = await create_account(db_session, budget, "Checking")
    ...
    # Creating the Liability entity must not change net worth: the liability is
    # already counted through its account ledger.
    await create_liability(db_session, budget, "Car", linked_account_id=loan.id)

    after_history, after_dashboard = await _net_worth_now(report_svc, budget.id)
    assert after_history == before_history
    assert after_dashboard == before_dashboard
    history = await report_svc.net_worth_history(budget.id, months=3)
    assert history[-1]["unmanaged_liability_total"] == Decimal("0")


async def test_history_before_first_snapshot_shows_no_liability(db_session):
    """A liability tracked starting last month must not rewrite older history."""
    report_svc = ReportService(db_session)
    user = await create_user(db_session)
    budget = await create_budget(db_session, user)
    checking = await create_account(db_session, budget, "Checking")
    ...
    liability = await create_liability(
        db_session, budget, "Medical", manual_balance=Decimal("900.00")
    )
    first_snapshot = TODAY.replace(day=1) - timedelta(days=10)  # in the previous month
    await create_liability_snapshot(db_session, liability, first_snapshot, Decimal("900.00"))

    history = await report_svc.net_worth_history(budget.id, months=6)

    oldest = history[0]
    assert oldest["unmanaged_liability_total"] == Decimal("0")
    assert oldest["net_worth"] == Decimal("5000.00")
    assert history[-1]["unmanaged_liability_total"] == Decimal("900.00")
    assert history[-1]["net_worth"] == Decimal("4100.00")
    previous_month = history[-2]
    assert previous_month["unmanaged_liability_total"] == Decimal("900.00")
...
async def test_api_response_carries_top_level_bucket(api_client, db_session):
    ...
    liability = await create_liability(
        db_session, budget, "Family", manual_balance=Decimal("450.00")
    )
    await create_liability_snapshot(db_session, liability, TODAY, Decimal("450.00"))

    resp = await api_client.get(f"/api/v1/{budget.id}/reports/net-worth")
    assert resp.status_code == 200
    body = resp.json()
    assert Decimal(str(body["unmanaged_liability_total"])) == Decimal("450.00")
    latest = body["points"][-1]
    assert Decimal(str(latest["net_worth"])) == Decimal("1550.00")
    assert Decimal(str(latest["unmanaged_liability_total"])) == Decimal("450.00")
