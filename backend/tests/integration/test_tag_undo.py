...
class TestTagUndo:
    ...
    async def test_delete_undo_hands_back_the_memberships_it_stripped(self, db_session, api_client):
        """Categories only — the payee half of this went with the payee tag
        routes. The delete record still snapshots `_payee_ids`, which is now
        always empty: the `payee_tags` table stays this release so undo of a
        record written before today still restores what it stripped."""
        budget, category, _payee = await _setup(db_session, api_client)
        tag = (await api_client.post(f"/api/v1/{budget.id}/tags", json={"name": "Splurge"})).json()
