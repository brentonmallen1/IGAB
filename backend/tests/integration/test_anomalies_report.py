...
async def test_spike_flags_with_exact_leave_one_out_zscore(db_session):
    ...
    await _spend_series(
        db_session, budget, checking, groceries,
    ...
    await create_transaction(
        db_session, budget, checking, "-100.00", months_ago(0),
        category=groceries, cleared="pending",
    )
    await create_transaction(
        db_session, budget, checking, "-50.00", months_ago(0),
        category=groceries, is_deleted=True,
    )
    ...
    await _spend_series(
        db_session, budget, checking, sys_cat,
...
async def test_threshold_parameter_bounds_detection(db_session):
    ...
    await _spend_series(
        db_session, budget, checking, dining,
...
async def test_guard_rails_silence_flat_and_small_dollar_baselines(db_session):
    ...
    await _spend_series(
        db_session, budget, checking, flat,
    ...
    await _spend_series(
        db_session, budget, checking, wobble,
...
async def test_low_side_anomaly_flags_with_direction_low(db_session):
    ...
    await _spend_series(
        db_session, budget, checking, fuel,
...
async def test_fewer_than_six_category_months_never_flags(db_session):
    ...
    await _spend_series(
        db_session, budget, checking, sparse,
