...
async def test_hidden_categories_are_still_offered(db_session, api_client):
    ...
    A real import put "Discover Savings" in YNAB's Hidden Categories group and
    tagged it Savings; the savings report has counted it ever since.
    """
    budget, _, made = await _budget_with(db_session, api_client, ["Discover Savings"])
    made["Discover Savings"].is_hidden = True
    await db_session.flush()
