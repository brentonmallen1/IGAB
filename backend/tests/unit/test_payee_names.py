...
class TestSamplesFromLegacy:
    def test_the_old_comma_string_splits_one_last_time(self) -> None:
        assert samples_from_legacy("NORTHWIND PAYROLL, northwind payroll , NORTHWIND PAYSERV") == [
            "NORTHWIND PAYROLL",
            "NORTHWIND PAYSERV",
        ]
...
class TestRankMatchPatterns:
    ...
    def test_one_stray_sample_does_not_blank_the_answer(self) -> None:
        # A bank name split on its own comma leaves "JANE" in the list; the
        # obvious pattern still comes back, 20 of 21.
        names = [f"NORTHWIND PAYSERV PAYROLL 2608{d:02d} 1000000000{d:02d}" for d in range(1, 21)]
        names.append("JANE")
        assert rank_match_patterns(["^NORTHWIND PAYSERV PAYROLL "], names, 3) == [
            "^NORTHWIND PAYSERV PAYROLL "
        ]
