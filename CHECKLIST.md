...
- [x] (M) Publish multi-arch (amd64/arm64) images to GHCR on tag/release (api + nginx-frontend) — 2026-08-12: `.github/workflows/release-images.yml` builds igab-api, igab-web, and igab-backup (new `scripts/Dockerfile`: postgres:16-alpine + age + the agent script baked in) on `v*` tags with semver + latest tags, QEMU/buildx, per-image gha cache. Frontend production image made self-contained: nginx config baked as an envsubst template with `API_UPSTREAM` env (default `api:8000`), compose no longer mounts nginx.conf; verified production builds use the relative `/api/v1` base (no VITE_API_URL baked). `APP_VERSION` build-arg stamps the release tag into the api image
- [x] (S) Docs for Unraid's "Docker Compose Manager" plugin driving our existing compose file as-is — 2026-08-12: `docs/unraid.md` Path A (clone + `.env` with `COMPOSE_PROFILES=production` + Unraid share paths), linked from README's Production Deployment section
- [x] (M) Unraid CA template repo — 2026-08-12: in-repo `unraid/` with CA v2 templates for igab-api, igab-web, igab-backup + `unraid/README.md` (add-repo and manual drop-in instructions, install order). Official postgres:16 pairing as `igab-db` on a custom `igab` docker network (container-name DNS); appdata layout `/mnt/user/appdata/igab/{postgres,attachments}` with backups deliberately on `/mnt/user/backups/igab` (different disk than the DB); SECRET_KEY/ADMIN_PASSWORD/PGPASSWORD masked+required; DATABASE_URL preset to the paired postgres. No VITE_API_URL/CORS config needed — web proxies /api same-origin (documented). Kept in-repo rather than a separate template repo; `<TemplateURL>`s point at raw.githubusercontent paths, easy to split later if submitted to CA proper
- _(pairs with)_ Encrypted backups — tracked under Data above; done 2026-08-10 (age encryption + in-app backups panel + restore protocol); `docs/unraid.md` references it
- [x] (S–M) In-app update notification for self-hosted installs (opt-in, off by default) — 2026-08-12: `update_check_enabled` app-setting (default "false"); `GET /system/update-status` answers locally without contacting GitHub when disabled (test asserts this), else compares `APP_VERSION` (workflow-stamped, "dev" locally) against the latest GitHub release with strict semver parse — dev/unparseable versions never nag; 6h in-process cache incl. failure caching. Settings → Updates panel (toggle + running version + release link banner); accent-dot badge next to Settings in the Sidebar, mirrored on the MoreSheet entry. 7 backend tests; toast skipped in favor of the passive badge (calm-by-default)
...
#### Postgres-enabled enhancements (2026-08-13 survey; none block the Unraid cutover — do not start before it)
_Postgres capabilities the app doesn't use yet, surveyed while weighing a SQLite port (decision: keep Postgres — money storage is `Numeric(19,4)` exact-decimal throughout and SQLite has no exact decimal type; the install overhead is solved by packaging instead). These are also the standing answer to "why Postgres": each is something SQLite can't do or does much worse._
- [ ] (S) pg_trgm fuzzy matching: `CREATE EXTENSION pg_trgm`, use `similarity()` to surface near-duplicate payees in merge suggestions with no regex pattern authored ("AMZN Mktp US" ≈ "Amazon.com") and to make payee/memo search typo-tolerant. Best effort-to-value on this list — do first
- [ ] (M) Live multi-device sync via LISTEN/NOTIFY → SSE: partner enters a transaction on their phone, it appears on the open desktop register without refresh (NOTIFY on write → SSE endpoint → React Query invalidation). The most tangible user-facing feature Postgres enables for a 2-person household; the backup agent's settings polling can ride the same mechanism. Schedule after the cutover proves out
- [ ] (M) Database-enforced invariants: translate the most critical integrity-suite checks (split sums = parent amount, transfer pairing) into constraint triggers / deferred constraints so violations roll back instead of landing and waiting to be detected — prevention on top of the existing detection suite. Fold in whenever the integrity suite next gets touched
- [ ] (M) Point-in-time recovery via WAL archiving (pgBackRest or wal-g): "restore to 2:47pm, right before the bad import" instead of last night's dump. Real safety upgrade but real operational complexity — current dump+retention is proportionate for a household; revisit if a restore ever loses meaningful data
- [ ] (S) Full-text search on memos/payees (`tsvector` + `websearch_to_tsquery`): word-stemming beats ILIKE for longer memos — marginal while memos stay short and the B2 token parser covers structured queries; bundle with the pgvector item if that ever happens rather than doing alone
- [ ] (exploration, no commitment) Expense knowledge graph: entities (payees/merchants, categories, tags, accounts) linked by transaction edges; embeddings (pgvector) or tsvector similarity could cluster related spending across category boundaries ("everything around the kitchen remodel" spanning Home Depot, permits, takeout spikes). Value unproven — before building anything, name 2–3 concrete questions the graph answers that reports/sankey/tag-filtering can't; if none survive scrutiny, drop it. Cheapest probe: a one-off notebook clustering payee embeddings on real post-cutover data to see if the clusters mean anything. First concrete use case if it pans out: retroactive project-membership suggestions (see Projects below)

#### Feature explorations (2026-08-13 brainstorm; post-cutover)
- [ ] (M) Projects — cross-category expense grouping: envelopes answer "where did the money come from" (monthly, category-siloed); projects answer "what was this all for" (episodic, cross-category — kitchen remodel spans Home Improvement + Dining Out + permit Fees). Build as a promotion of tags, not a parallel mechanism (same philosophy as the rules-engine note): a project is a tag plus lifecycle (active/done status, optional cost estimate to track against, date range) and a dedicated view (total cost, category/payee breakdown, spend-over-time, attached receipts in one place). Active projects surface as one-tap toggles in QuickAdd and the editor; the knowledge-graph embedding probe above can later suggest retroactive membership
- [ ] (M–L) Spending focus areas — habit coaching for user-chosen scopes (category, payee(s), or tag; "type of good" finer than payee waits for the AI layer). Opt-in; goal is a soft per-period cap or reduce-by-% vs trailing 3-month average. Design principles (behavior-change grounded, must survive into implementation): own-baseline framing beats absolute caps ("30% below your DoorDash average" — self-calibrating, less resented); timing beats volume — the at-entry nudge in QuickAdd (e.g. "3rd DoorDash this week · $86 this month, goal $60") and the sync-review moment are the high-leverage points, a banner days later is commentary not intervention; shame causes avoidance — one notification at threshold crossing, never a standing red banner (calm-by-default brand; people who feel judged stop opening the app, the worst outcome for a budget tool); weekly digest leads with wins; quiet streaks ("4 weeks under goal"). Distinct from savings goals/targets: those are allocation, this is behavior
- [ ] (M) Chronic-overspending report — categories that routinely need mid-month money moved in to cover. Data caveat: covering overspending destroys the evidence (assigned rises, month ends looking healthy), so month-end-negative alone only catches uncovered overspending. Two-part approach: (a) heuristics over existing B3 snapshots work today — activity exceeded target in N of last M months, final assigned consistently above target (top-up signature); (b) a small timestamped assignment-event log makes it precise (detect "assigned raised after activity already exceeded it", plus which categories chronically donate the bailout money) but only sees history from when it ships — add the log early so data accumulates even if the report comes later. Framing: chronic overspending is a forecasting problem, not a discipline problem — punchline is "Groceries needed a median $85 top-up in 8 of 10 months → suggested target $635 (p75 of trailing actual)" with one-click apply, not a wall of red. Pairs with focus areas: this is the allocation-side fix (raise the dishonest budget), focus areas are the behavior-side fix (curb the spending); per category the report can route to either
